	...
	if ( member1 != null && userId > 0 ) {
		...
		if ( CommonUtil.isNotEmpty( comment ) ) {
			...
			if ( giveType == 1 ) {//送积分
			    ...
			    memberPayService.updateJifenAndFenBiByPinglu( memberId, give.getNum(), 0d );
			} else if ( giveType == 2 ) {//送粉币
			    memberPayService.updateJifenAndFenBiByPinglu( memberId, 0, give.getNum().doubleValue() );
