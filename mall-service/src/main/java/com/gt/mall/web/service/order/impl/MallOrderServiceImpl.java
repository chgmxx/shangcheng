...
public class MallOrderServiceImpl extends BaseServiceImpl< MallOrderDAO,MallOrder > implements MallOrderService {
	...
	if ( CommonUtil.isNotEmpty( type ) && count > 0 ) {
		...
		if ( order.getOrderPayWay() == 6 ) {//到店提货
			...
			//todo 调用彭江丽接口，根据会员id查询会员信息
			//member = memberMapper.selectByPrimaryKey( order.getBuyerUserId() );
	...
	if ( CommonUtil.isNotEmpty( orderList ) ) {
		...
		// todo 调用陈丹的接口，字典相关
		Map< String,Object > expressMap = null;//dictService.value( "1092", maps.get( "express_id" ).toString() );
		if ( CommonUtil.isNotEmpty( expressMap ) ) {
		    orderMap.put( "expressName", expressMap.get( "item_value" ) );
		}
	    }
	}
	...
	WxPublicUsers wpUser = new WxPublicUsers();
	try {
	    //todo 调用小屁孩接口 根据粉丝id查询公众号信息
	    //wpUser = mallOrderDAO.getWpUser( memberId );
	} catch ( Exception e ) {
	    e.printStackTrace();
	}
	return wpUser;
	...
	if ( CommonUtil.isNotEmpty( member.getBusid() ) ) {
	    try {
		//todo 调用陈丹接口，socket相关
		//socke.sendMessage( member.getBusid(), url );   //订单成功推送消息
	    } catch ( Exception e ) {
		e.printStackTrace();
		logger.error( "消息推送异常：" + e.getMessage() );
	    }
...
//				smsService.sendMsg( oldApiSms );
	...
	//todo 调用陈丹接口口，根据商家id查询总账号id
	int userPId = 0;//dictService.pidUserId( order.getBusUserId() );//查询商家总账号
	//todo 调用陈丹接口，判断商家是否有进销存
	...
	if ( CommonUtil.isNotEmpty( order.getMallOrderDetail() ) && order.getOrderPayWay() != 5 && order.getOrderPayWay() != 7 ) {
		...
		//todo 调用彭江丽接口  判断用户是否是会员
		boolean isMember = false;//memberPayService.isMemember( order.getBuyerUserId() );
		...
		/*if ( isMember || CommonUtil.isNotEmpty( duofenCoupon ) ) {
		    try {
			if ( CommonUtil.toDouble( order.getOrderMoney() ) > 0 && isMember ) {
			    if ( order.getOrderPayWay() != 4 && order.getOrderPayWay() != 8 ) {//积分消费不赠送物品
				List idList = mallOrderDAO.selectOrderPid( order.getId() );
				if ( idList.size() > 0 ) {//多条订单
				    for ( int i = 0; i < idList.size(); i++ ) {
					JSONObject obj = JSONObject.fromObject( idList.get( i ).toString() );
					Integer id = Integer.parseInt( obj.get( "id" ).toString() );
					MallOrder order1 = mallOrderDAO.getOrderById( id );
					//todo 调用彭江丽接口，根据订单号添加赠送物品记录 延迟送
					memberPayService.findGiveRuleDelay( member.getPhone(), order1.getOrderNo() );
				    }
				} else {//单条订单
				    //todo 调用彭江丽接口，根据订单号添加赠送物品记录 延迟送
				    memberPayService.findGiveRuleDelay( member.getPhone(), newOrderNo );
				}
			    }
			}
		    } catch ( Exception e ) {
			logger.error( "订单完成，赠送物品异常：" + e.getMessage() );
			e.printStackTrace();
		    }
		}*/
	    }
	    ...
	    /*if ( proTypeId == 2 ) {
		//todo 调用彭江丽接口  会员卡购买记录
		memberPayService.buyCard( member, order.getOrderMoney(), memberType );//会员卡购买记录
	    }*/
	...
	if ( CommonUtil.isNotEmpty( orders.getFlowPhone() ) && flowId > 0 ) {
	    //todo 调用小屁孩  流量相关接口
	    //BusFlow flow = busFlowService.selectById( flowId );
	    List< Map< String,Object > > czlist = new ArrayList<>();
	    Map< String,Object > czMap = new HashMap<>();
	    czMap.put( "mobile", orders.getFlowPhone() ); //充值手机号
	    //todo 流量相关
	    //czMap.put( "prizeCount", flow.getType() ); //流量数
	    czMap.put( "memberId", member.getId() ); //用户Id
	    czMap.put( "publicId", pbUser.getId() ); //商家公众号Id
	    czMap.put( "busId", orders.getBusUserId() ); //根据平台用户Id获取微信订阅号用户信息
	    czMap.put( "id", orders.getId() ); //一条中信息Id
	    czlist.add( czMap );
	    try {
		//todo 调用小屁孩流量充值接口
		Map< String,Object > flowMap = null;//MobileLocationUtil.adcServices( czlist, 101, "t_mall_order" );
		if ( flowMap.get( "code" ).toString().equals( "1" ) ) {//充值成功
		    //todo 调用小屁孩流量充值成功后的回调接口
     ...
     * 添加订单支付成功记录(总记录表)
     */
    private void addUserConsume( MallOrder orderno, Member member, double fenbi, double integral, Map< String,Object > param, int discount, String orderNo,
		    WxPublicUsers wxPublicUser ) {
	//todo 调用彭江丽 支付成功后的记录
	/*Integer memType = orderno.getMemCardType();
	if ( CommonUtil.isEmpty( memType ) ) {
	    memType = 6;
	}
	int uctype = 6;//游客消费
	if ( memType == 1 ) {
	...
	MallStore store = mallStoreDAO.selectById( orderno.getShopId() );
	UserConsume consume = new UserConsume();
	if ( CommonUtil.isNotEmpty( wxPublicUser ) ) {
	    consume.setPublicId( wxPublicUser.getId() );//公众号id
	}
	consume.setBususerid( member.getBusid() );//商户id
	Card card = null;
	if ( uctype != 6 ) {
	    card = memberPayService.findCardByMemberId( orderno.getBuyerUserId() );
	    consume.setMcid( card.getMcId() );
	    consume.setGtId( card.getGtId() );
	    consume.setCtid( card.getCtId() );
	}
	consume.setDiscountmoney( CommonUtil.toDouble( 0 ) );//实付订单金额
	//积分消费
	if ( orderno.getOrderPayWay() == 4 ) {
	    uctype = 5;
	    consume.setIntegral( (int) orderno.getOrderMoney() );
	    consume.setRecordtype( Byte.valueOf( "0" ) );//记录类型 0积分记录
	} else if ( orderno.getOrderPayWay() == 8 ) {//粉币消费
	    uctype = 19;
	    consume.setFenbi( orderno.getOrderMoney() );
	    consume.setRecordtype( Byte.valueOf( "4" ) );//记录类型 4积分记录
	} else {//非积分记录
	    consume.setRecordtype( Byte.valueOf( "2" ) );//记录类型 2消费记录
	    consume.setDiscountmoney( orderno.getOrderMoney() );//实付订单金额
	}
	double totalMoney = 0;
	if ( CommonUtil.isNotEmpty( orderno.getOrderOldMoney() ) ) {
	    totalMoney += orderno.getOrderOldMoney();
	}
	if ( CommonUtil.isNotEmpty( orderno.getOrderFreightMoney() ) ) {
	    totalMoney += orderno.getOrderFreightMoney();
	}
	if ( totalMoney < orderno.getOrderMoney() ) {
	    totalMoney = orderno.getOrderMoney();
	}
	consume.setTotalmoney( totalMoney );//订单原价（折扣前的价格总价）

	if ( fenbi > 0 ) {//粉币抵扣
	    consume.setFenbi( fenbi );
	}
	if ( integral > 0 ) {//积分抵扣
	    consume.setIntegral( (int) integral );
	}
	consume.setModuletype( Byte.valueOf( "0" ) );//模块类型 0 商城

	consume.setMemberid( orderno.getBuyerUserId() );//买家id
	consume.setStoreid( store.getWxShopId() );//(店铺Id 改为微信门店id
	if ( uctype == 2 && discount < 100 && discount > 0 ) {
	    consume.setDiscount( discount );//折扣数
	}
	if ( CommonUtil.isEmpty( consume.getDiscount() ) ) {
	    consume.setDiscount( 100 );//折扣数
	}
	consume.setFreightmoney( orderno.getOrderFreightMoney() );//运费
	consume.setUctype( Byte.valueOf( uctype + "" ) );//消费类型
	consume.setOrderid( orderno.getId() );//总订单id

	int payWay = orderno.getOrderPayWay();//支付方式
	if ( payWay == 2 ) {//货到付款
	    payWay = 4;
	} else if ( payWay == 3 ) {//储值卡支付
	    payWay = 5;
	} else if ( payWay == 4 ) {//积分支付
	    payWay = 6;
	} else if ( payWay == 5 ) {//扫码支付
	    payWay = 1;
	} else if ( payWay == 6 ) {//到店支付
	    payWay = 8;
	} else if ( payWay == 7 ) {//找人代付
	    payWay = 9;
	} else if ( payWay == 8 ) {//粉币代付
	    payWay = 7;
	} else if ( payWay == 9 ) {//支付宝支付
	    payWay = 0;
	}
	consume.setUctable( "t_mall_order" );//订单表名
	consume.setCreatedate( new Date() );//创建时间
	consume.setPaymenttype( Byte.valueOf( payWay + "" ) );//支付方式
	if ( payWay != 8 || payWay != 9 ) {
	    consume.setPaystatus( Byte.valueOf( "1" ) );//支付状态 1 已支付
	} else {//到店支付，找人代付  要等商家确认收款才改变支付状态
	    consume.setPaystatus( Byte.valueOf( "0" ) );//支付状态 0 未支付
	}
	consume.setOrdercode( orderNo );//订单编号
	if ( CommonUtil.isNotEmpty( param ) ) {
	    String code = null;
	    String useCardId = null;
	    boolean isUseDuofen = false;
	    if ( CommonUtil.isNotEmpty( param.get( "duofenCoupon" ) ) ) {
		String c = param.get( "duofenCoupon" ).toString();
		String[] str = c.split( "\\|" );
		for ( int i = 0; i < str.length; i++ ) {
		    if ( str[i].contains( orderno.getId() + "" ) ) {
			isUseDuofen = true;
			code = str[i].substring( 0, str[i].indexOf( "-" ) );
			useCardId = str[i].substring( str[i].indexOf( "=" ) + 1, str[i].length() );
		    }
		}
		if ( isUseDuofen ) {
		    consume.setDiscountdepict( code );//使用优惠券code
		    consume.setCardtype( Byte.valueOf( "1" ) );//优惠券类型（1多粉  0微信）
		    consume.setDvid( Integer.parseInt( useCardId ) );//优惠券Id
		}
	    }
	    if ( CommonUtil.isNotEmpty( param.get( "wxCoupon" ) ) ) {
		code = param.get( "wxCoupon" ).toString();
		if ( code.contains( orderno.getId() + "" ) ) {
		    code = code.substring( 0, code.indexOf( "-" ) );
		    consume.setDiscountdepict( code );//使用优惠券code
		}
		consume.setCardtype( Byte.valueOf( "0" ) );//优惠券类型（1多粉  0微信）
	    }
	}
	if ( CommonUtil.isNotEmpty( orderno.getBuyerUserType() ) ) {
	    consume.setDatasource( orderno.getBuyerUserType() );
	}
	if ( CommonUtil.isNotEmpty( orderno.getOrderType() ) ) {
	    if ( orderno.getOrderType() > 0 ) {
		consume.setModuleordertype( CommonUtil.toInteger( "100" + orderno.getOrderType() ) );
	    }
	}
	//添加消费记录
	...
	if ( orderno.getOrderPayWay() == 4 && card != null && member != null && CommonUtil.isNotEmpty( consume.getIntegral() ) ) {//积分兑换商品消费记录
	    isXf = false;
	    memberPayService.saveCardRecordNew( card.getMcId(), Byte.valueOf( "2" ), consume.getIntegral() + "积分", "商城积分兑换商品消费", card.getBusid(),
			    "", card.getCtId(), -consume.getIntegral() );
	}
	if ( orderno.getOrderPayWay() == 8 && card != null && member != null && CommonUtil.isNotEmpty( consume.getFenbi() ) ) {//粉币兑换商品消费记录
	    isXf = false;
	    memberPayService.saveCardRecordNew( card.getMcId(), Byte.valueOf( "3" ), consume.getFenbi() + "粉币", "商城粉币兑换商品消费", card.getBusid(),
			    "", card.getCtId(), -consume.getFenbi() );
	}
	if ( fenbi > 0 && card != null && member != null ) {//添加粉币消费记录
	    memberPayService.saveCardRecordNew( card.getMcId(), Byte.valueOf( "3" ), fenbi + "粉币", store.getStoName() + "商城粉币消费", card.getBusid(),
			    "", card.getCtId(), -fenbi );
	}
	if ( integral > 0 && card != null && member != null ) {//添加积分消费记录
	    memberPayService.saveCardRecordNew( card.getMcId(), Byte.valueOf( "2" ), integral + "积分", store.getStoName() + "商城积分消费", card.getBusid(),
			    "", card.getCtId(), -integral );
	}
	if ( isXf && orderno.getOrderMoney() > 0 ) {//
	    memberPayService.saveCardRecordNew( card.getMcId(), Byte.valueOf( "1" ), orderno.getOrderMoney() + "元", "商城消费", card.getBusid(),
			    "", card.getCtId(), -orderno.getOrderMoney() );
	}*/
    }
	...
	try {
	   ...
	   /* Member member = null;//memberService.findById( orderReturn.getUserId() );
	    WxPublicUsers wx = null;
	    if ( CommonUtil.isNotEmpty( member.getPublicId() ) ) {
		//todo 调用小屁孩的接口 根据公众号的id查询公众号的信息
		wx = wxPublicUserMapper.selectByPrimaryKey( member.getPublicId() );
	    }
	...
	if ( orderReturn != null ) {
		...
		if ( oObj != null && ( orderReturn.getStatus() == 1 || orderReturn.getStatus() == 5 ) ) {// 同意退款
			...
			if ( CommonUtil.isNotEmpty( oReturn.getReturnFenbi() ) ) {
			    //todo 调用陈丹的接口，根据商家id查询商家信息
			    BusUser busUser = null;//busUserMapper.selectByPrimaryKey( order.getBusUserId() );
			...
			if ( order != null && flags ) {
				...
				if ( Double.parseDouble( oReturn.getRetMoney().toString() ) > 0 ) {//退款金额大于0，则进行微信退款
				    //todo 调用小屁孩接口，根据订单号查询支付订单信息
				    WxPayOrder wxPayOrder = null;//wxPayOrderMapper.selectByOutTradeNo( payMap.get( "orderNo" ).toString() );
					...
					map.put( "outRefundNo", oReturn.getReturnNo() );// 商户退款单号(系统生成)
					map.put( "totalFee", wxPayOrder.getTotalFee() );// 总金额
					map.put( "refundFee", oReturn.getRetMoney() );// 退款金额
				...
				//todo 调用彭江丽接口，储值卡退款
				Map< String,Object > payResultMap = null;//memberPayService.chargeBack( oReturn.getUserId(), retMoney );
				...
				if ( retMoney > 0 && flag ) {//退款金额大于0，则进行微信退款
					...
					//todo 调用小屁孩接口 根据订单号查询支付订单信息
					WxPayOrder wxPayOrder = null;//wxPayOrderMapper.selectByOutTradeNo( daifu.getDfOrderNo() );
					if ( wxPayOrder.getTradeState().equals( "SUCCESS" ) ) {
					    map.put( "appid", pUser.getAppid() );// 公众号
					    map.put( "mchid", pUser.getMchId() );// 商户号
					    map.put( "sysOrderNo", wxPayOrder.getOutTradeNo() );// 系统订单号
					...
					if ( Double.parseDouble( oReturn.getRetMoney().toString() ) > 0 ) {//退款金额大于0，则进行微信退款
						...
						appletMap.put( "outRefundNo", oReturn.getReturnNo() );//退款单号
						appletMap.put( "totalFee", wxPayOrder.getTotalFee() );//总金额
						appletMap.put( "refundFee", oReturn.getRetMoney() );//退款金额
						appletMap.put( "busId", order.getBusUserId() );
						...
						Map< String,Object > openMap = null;//daoUtil.queryForMap( sql );
						appletMap.put( "mchid", openMap.get( "mch_id" ) );// 商户号
						appletMap.put( "appid", openMap.get( "appid" ) );// 公众号
						logger.info( "appletWxReturnParams:" + JSONObject.fromObject( appletMap ).toString() );
						//todo 调用小屁孩接口   小程序退款
						Map< String,Object > resultmap = null;//payService.memberPayRefundByWxApplet( appletMap );
						if ( CommonUtil.isNotEmpty( resultmap ) ) {
						    if ( resultmap.get( "code" ).toString().equals( "1" ) ) {
							//退款成功修改退款状态
							updateReturnStatus( pUser, oReturn, orderReturn, order );//微信退款
	...
	//todo 调用彭江丽的几口  退款成功后添加退款记录
	/*if ( memberPayService.isMemember( order.getBuyerUserId() ) ) {//是会员才添加退款记录
	    //通过店铺id查询店铺信息
	    Store store = mallStoreDAO.selectByPrimaryKey( oReturn.getShopId() );
	    //查询订单的消费记录
	    UserConsume consume = userConsumeMapper.findByOrderCode1( order.getOrderNo() );
	    int xiaofeiId = 0;
	    ...
	    Object memCardType = order.getMemCardType();
	    if ( null == memCardType || memCardType == "" || memCardType.equals( "" ) ) {
		memCardType = "0";
	    }
	    int uctype = 6;
	    switch ( Integer.parseInt( memCardType.toString() ) ) {
		case 1:
		    uctype = 0;//积分卡消费
		    break;
		case 2:
	    ...
	    Card card = null;
	    if ( uctype != 6 ) {
		card = memberPayService.findCardByMemberId( order.getBuyerUserId() );
		consume.setMcid( card.getMcId() );
		consume.setGtId( card.getGtId() );
		consume.setCtid( card.getCtId() );
	    }
	    int shopId = store.getWxShopId();
	    if ( shopId <= 0 ) {
		shopId = store.getId();
	    }
	    ...
	    consume.setBususerid( order.getSellerUserId() );//商户id
	    consume.setModuletype( Byte.valueOf( "0" ) );//模块类型 0 商城
	    consume.setStoreid( shopId );//店铺id
	    //			consume.setIsparent(isBrachStore);//是否是子店铺

	    consume.setMemberid( oReturn.getUserId() );//买家id
	    consume.setRecordtype( Byte.valueOf( "3" ) );//记录类型 3退款记录
	    consume.setUctype( Byte.valueOf( "12" ) );//消费类型
	    consume.setTotalmoney( order.getOrderMoney() );//退款金额 应付退款
	    consume.setDiscountmoney( Double.valueOf( oReturn.getRetMoney().toString() ) );//实付退款
	    if ( CommonUtil.isNotEmpty( oReturn.getReturnJifen() ) ) {
		if ( oReturn.getReturnJifen() > 0 ) {
		    consume.setIntegral( oReturn.getReturnJifen() );
		}
	    }
	    if ( CommonUtil.isNotEmpty( oReturn.getReturnFenbi() ) ) {
		if ( oReturn.getReturnFenbi() > 0 ) {
		    consume.setFenbi( oReturn.getReturnFenbi() );
		}
	    }
	    if ( order.getOrderPid() == 0 ) {
		consume.setOrderid( order.getId() );//订单id
	    } else {
		consume.setOrderid( order.getOrderPid() );//总订单id
	    }
	    int payWay = order.getOrderPayWay();//支付方式
	    if ( payWay == 2 ) {//货到付款
		payWay = 4;
	    } else if ( payWay == 3 ) {//储值卡支付
		payWay = 5;
	    ...
	    consume.setOrdercode( oReturn.getReturnNo() );//订单号微信或支付宝或自定义
	    //添加退款记录
	    consumeMapper.insert( consume );
		...
		if ( xiaofeiId > 0 ) {
		    ...
		    consumeRefundMapper.insert( refund );
		}

		//判断是否是会员 储值卡不进回调方法
		if ( memberPayService.isMemember( oReturn.getUserId() ) && order.getOrderPayWay() != 3 && CommonUtil.isNotEmpty( consume.getMcid() ) ) {
		    if ( consume.getMcid() > 0 ) {
			memberPayService.refundBack( oReturn.getReturnNo() );//退款成功 回调 添加card操作记录
		    }
		}
	    }
			*//*Card card = memberPayService.findCardByMemberId(order.getBuyerUserId());*//*
	    Member member = memberService.findById( order.getBuyerUserId() );
	    if ( oReturn.getReturnFenbi() > 0 && card != null && member != null ) {//添加粉币消费记录
		memberPayService.saveCardRecordNew( card.getMcId(), Byte.valueOf( "3" ), oReturn.getReturnFenbi() + "粉币", "商城订单退款粉币记录", card.getBusid(),
				"", card.getCtId(), -oReturn.getReturnFenbi() );
	    }
	}*/
	...
	if ( productId > 0 ) {
	    ...
	    if ( product != null ) {
		//todo 调用陈丹接口  查询主账号id
		int userPId = 0;//dictService.pidUserId( product.getUserId() );//通过用户名查询主账号id
		//todo 调用陈丹接口  判断是否有进销存
		long isJxc = 0;//erpLoginOrMenusService.isjxcCount( "8", userPId );//判断商家是否有进销存 0没有 1有
	...
	//todo 调用接口 判断浏览器类型
	Integer browser = 0;//CommonUtil.judgeBrowser( request, member.getBusid() );
	if ( browser != 1 ) {
	    browser = 2;
	}
	...
	//查询粉币抵扣的比例
	//todo 调用陈丹接口  字典相关
	List< Map< String,Object > > currencyList = null;//dictService.getDictList( "1058" );
...
*/
	...
	if ( CommonUtil.isNotEmpty( order ) ) {
		...
		//todo 调用彭江丽接口  根据粉丝id查询会员信息
		Member member = null;//memberMapper.selectByPrimaryKey( order.getBuyerUserId() );
		updateMemberJifen( useFenbi, useIntegral, member );
	    }
	    /*if ( CommonUtil.isNotEmpty( order.getMallOrderDetail() ) ) {
		for ( MallOrderDetail detail : order.getMallOrderDetail() ) {
			...
			*//*if ( detail.getCardReceiveId() > 0 ) {
			    //todo 调用彭江丽接口   商场支付成功回调 分配卡券
			    duofenCardService.successPayBack( detail.getCardReceiveId(), detail.getDetProNum(), order.getBuyerUserId() );
			}*//*
		    }
		}
	    }*/
	...
	//todo 调用小屁孩   根据商家id查询公众号信息
	WxPublicUsers publicUser = null;//wxPublicUserMapper.selectByUserId( order.getSellerUserId() );//通过商家id查询商家公众号id
	MallPaySet paySet = new MallPaySet();
	if ( CommonUtil.isNotEmpty( publicUser ) ) {
	...
	//todo 调用彭江丽接口根据粉丝id查询粉丝信息
	Member member = null;//memberService.findById( order.getBuyerUserId() );//查询买家信息
	String memberName = "";
	String memberPhone = "";
	String address = "";
	String shopName = "";
...
//		    }
	...
	if ( !params.containsKey( "oldMemberIds" ) ) {
