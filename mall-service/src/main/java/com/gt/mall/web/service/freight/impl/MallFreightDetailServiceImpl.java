package com.gt.mall.web.service.freight.impl;
...
public class MallFreightDetailServiceImpl extends BaseServiceImpl<MallFreightDetailDAO, MallFreightDetail> implements MallFreightDetailService {
    ...
    public void editFreightDetail(Map<String, Object> params, int freightId) {
        ...
        if (!CommonUtil.isEmpty(params.get("delDetail"))) {
            delDetailMap = (Map<String, Object>) JSONObject.toBean(JSONObject.fromObject(params.get("delDetail")), Map.class);
        }
        // 需要删除的物流配送区域
        if (!CommonUtil.isEmpty(params.get("delPro"))) {
            delProMap = (Map<String, Object>) JSONObject.toBean(
                    JSONObject.fromObject(params.get("delPro")), Map.class);
        }

        if (!CommonUtil.isEmpty(params.get("detail"))) {
            // 物流详情
            List<MallFreightDetail> detailList = (List<MallFreightDetail>) JSONArray
                    .toList(JSONArray.fromObject(params.get("detail")), MallFreightDetail.class);
    ...
    private void updateFreightProvince(MallFreightDetail detail, int freightId, Map<String, Object> delProMap) {
        if (detail.getProvinceList() != null && detail.getId() != null) {
            List<MallFreightProvinces> proList = (List<MallFreightProvinces>) JSONArray
                    .toList(JSONArray.fromObject(detail.getProvinceList()), MallFreightProvinces.class);
