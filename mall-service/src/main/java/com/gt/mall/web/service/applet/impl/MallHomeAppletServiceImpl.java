...
import com.gt.mall.web.service.product.MallProductParamService;
...
public class MallHomeAppletServiceImpl extends BaseServiceImpl<MallAppletImageDAO, MallAppletImage> implements MallHomeAppletService {
    ...
    public List<Map<String, Object>> selectGroupsByShopId(Map<String, Object> params) {
        ...
        //TODO  需调用 MallProductGroupDAO.selectGroupsByShopId()方法
        List<Map<String, Object>> groupList = null;
//                MallProductGroupDAO.selectGroupsByShopId(params);
    ...
    private Map<String, Object> productGetPrice(Map<String, Object> map1, Map<String, Object> params, double discount) {
        ...
        if (CommonUtil.isNotEmpty(imageUrl)) {
            proMap.put("image_url", PropertiesUtil.getResourceUrlPrefix() + imageUrl);
        }
...
//          erpLoginOrMenusService.isjxcCount("8", userPId);//判断商家是否有进销存 0没有 1有
        ...
        //计算批发价
        //TODO 需调用  pifaApplyService.isPifa 判断是否批发商
        boolean isPifa = true;
//                pifaApplyService.isPifa(member);
        if (isPifa) {
...
//            }
            ...
            //TODO 需调用  orderService.selectShipAddress(addressParams);获取默认地址
        ...
        if (addressList != null && addressList.size() > 0) {
            ...
            if (CommonUtil.isNotEmpty(addressMap)) {
                String address = CommonUtil.toString(addressMap.get("pName")) +
                        ...
                        CommonUtil.toString(addressMap.get("aName")) +
                        CommonUtil.toString(addressMap.get("mem_address"));
                productMap.put("address", address);
                productMap.put("address_id", addressMap.get("id"));
            }

        }
        productMap.put("freightPrice", freightService.getFreightByProvinces(params, addressMap, product.getShopId(), proPrice * discount, CommonUtil.toDouble(product.getProWeight())));

        //查询店铺信息
        //TODO 需调用 pageService.shopmessage 获取店铺信息
        Map<String, Object> shopMap = null;
//                pageService.shopmessage(product.getShopId());
        if (CommonUtil.isNotEmpty(shopMap)) {
            //店铺图片
            if (CommonUtil.isNotEmpty(shopMap.get("stoPicture"))) {
                productMap.put("shopImage", PropertiesUtil.getResourceUrlPrefix() + shopMap.get("stoPicture"));
            } else {
                productMap.put("shopImage", PropertiesUtil.getResourceUrlPrefix() + shopMap.get("sto_picture"));
            }
            //店铺名称
        ...
        if (imageList != null && imageList.size() > 0) {
            for (Map<String, Object> map : imageList) {
                map.put("image_url", PropertiesUtil.getResourceUrlPrefix() + map.get("image_url"));
        ...
        if (list != null && list.size() > 0) {
            for (Map<String, Object> map : list) {
                ...
                if (CommonUtil.isNotEmpty(map.get("shopPicture"))) {
                    shopMap.put("shopImage", PropertiesUtil.getResourceUrlPrefix() + map.get("shopPicture"));
                } else {
                    shopMap.put("shopImage", PropertiesUtil.getResourceUrlPrefix() + map.get("stoPicture"));
                }
