...
public class MallOrderAppletServiceImpl extends BaseServiceImpl<MallAppletImageDAO, MallAppletImage> implements MallOrderAppletService {
    ...
    public PageUtil getOrderList(Map<String, Object> params) {
        //TODO 需关连 memberPayService.findMemberIds() 会员方法
        List<Integer> memberList = null;
//                memberPayService.findMemberIds(CommonUtil.toInteger(params.get("memberId")));//查询会员信息
    ...
    private Map<String, Object> getOrderParam(MallOrder order) {
        ...
        for (Object dObject2 : arr) {
            ...
            if (isReturn == 1 && groupIsReturn == 0) {
                if (((orderStatus.equals("2") || orderStatus.equals("3")) && status == -3) || (updateDay > 0 && updateDay < returnDay && orderStatus.equals("4") && status == -3)) {
            ...
            detailMap.put("product_image", PropertiesUtil.getResourceUrlPrefix() + dObj.get("productImageUrl"));
    ...
    public Map<String, Object> getOrderDetail(Map<String, Object> params) {
        ...
        if (CommonUtil.isNotEmpty(order)) {
            ...
            if (CommonUtil.isNotEmpty(order.getReceiveId())) {
                ...
                //TODO 需关连 orderDAO.selectShipAddress(params);地址方法
                List<Map<String, Object>> list = null;
//                        orderDAO.selectShipAddress(params);
                if (list != null && list.size() > 0) {
...
//                    storeMapper.selectByStoreId(params);
            if (CommonUtil.isNotEmpty(storeMap)) {
                ...
                if (CommonUtil.isNotEmpty(storeMap.get("shopPicture"))) {
                    resultMap.put("shop_image", PropertiesUtil.getResourceUrlPrefix() + storeMap.get("shopPicture"));
                } else {
                    resultMap.put("shop_image", PropertiesUtil.getResourceUrlPrefix() + storeMap.get("stoPicture"));
                }
            }
            ...
            if (list != null && list.size() > 0) {
                for (MallOrderDetail detail : list) {
                    ...
                    detailMap.put("product_image", PropertiesUtil.getResourceUrlPrefix() + detail.getProductImageUrl());
    ...
    public Map<Object, Object> orderGoPay(Map<String, Object> params, String url) throws Exception {
        ...
        if (error < 0 && msg.equals("")) {
            ...
            if (order.getOrderPayWay() == 10) {
                //TODO 需关连memberService.findById() 会员 方法
                Member member = null;
...
//                resultMap.putAll(parameters);
    ...
    public Map<String, Object> confirmReceipt(Map<String, Object> params) {
        ...
        //TODO  orderService.upOrderNoOrRemark(params); 方法
        int count = 0;
...
//            resultMap.put("comList", comList);
    ...
    public Map<String, Object> submitReturnOrder(Map<String, Object> params) {
        ...
        //TODO   orderService.addOrderReturn(orderReturn);
        boolean flag = true;
//                orderService.addOrderReturn(orderReturn);
        if (flag) {
            resultMap.put("code", 1);
        } else {
            resultMap.put("code", -1);
...
//        return resultMap;
    ...
    public Map<String, Object> toSubmitOrder(Map<String, Object> params) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        int memberId = CommonUtil.toInteger(params.get("memberId"));
        //TODO 会员信息 memberService.findById(memberId);
        Member member = null;
//                memberService.findById(memberId);
        ...
        //TODO 会员信息 memberPayService.findMemberIds(memberId)
        List<Integer> memberList = null;
//                = memberPayService.findMemberIds(memberId);//查询会员信息
        ...
        //TODO 会员地址  mOrderMapper.selectShipAddress
...
//                    storeMapper.selectByStoreId(map);
    ...
    public Map<String, Object> submitOrder(Map<String, Object> params) {
        ...
        //TODO 会员 memberService.findById()
        Member member = null;
//               memberService.findById(memberId);
...
//                wxPublicUsersMapper.selectByUserId(member.getBusid());
        ...
        int memType = 0;
        //TODO 会员 memberPayService.isMemember() isCardType()
...
//                        memberPayService.updateMemberIntergral(null, member.getId(), CommonUtil.toInteger(-orderMoney));
                if (CommonUtil.isNotEmpty(payRresult.get("result"))) {
                    if (CommonUtil.toString(payRresult.get("result")).equals("2")) {
                        //TODO  mOrderService.paySuccessModified()
//                        mOrderService.paySuccessModified(params, member);//修改库存和订单状态
                    }
                }
            }
            if (orderPayWay == 2) {
...
//        return parameters;
    ...
    public Map<String, Object> productBuyNow(Map<String, Object> params) {
        ...
        int memberId = CommonUtil.toInteger(params.get("memberId"));
        //TODO 会员 memberService.findById(memberId);
        Member member = null;
//                memberService.findById(memberId);
        int memType = 0;
        //TODO 会员 memberPayService.isMemember() isCardType()
