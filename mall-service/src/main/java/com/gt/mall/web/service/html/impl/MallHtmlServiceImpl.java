...
import com.gt.mall.web.service.groupbuy.impl.MallGroupBuyPriceServiceImpl;
...
//        BusUser obj = CommonUtil.getLoginUser(request);//获取登录信息
    ...
    public void addorUpdateSave(MallHtml obj, BusUser user) {
        ...
        if (CommonUtil.isEmpty(obj.getId()) || obj.getId() == 0) {
            obj.setDataBg("[{}]");
            obj.setDataJson("[[]]");
            obj.setDataTransverse(" [{'data':[[],[],[]],'bg':[{ 'background':'#fff',},{ 'background':'#fff',},{ 'background':'#fff',}],'attr':{'width': 380,'height': 500,'top': 62,'left': 0,},'dataTransverse_on':false},]");
