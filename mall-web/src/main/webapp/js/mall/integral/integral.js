...
function choosePro() {
    ...
    if (shopId != null && shopId != "") {
        // parentOpenIframe("选择商品", "600px", "480px", "/mGroupBuy/getProductByGroup.do?shopId=" + shopId + "&defaultProId=" + defaultProId + "&isIntegral=1");//check==0代表多选，check==1代表单选
        SonScrollTop(0);
        setTimeout(function () {
            layer.open({
                type: 2,
                ...
                shade: [0.1, "#fff"],
                content:  "/mGroupBuy/getProductByGroup.do?shopId=" + shopId + "&defaultProId=" + defaultProId + "&isIntegral=1"
            });
        }, timeout);
    } else {
        layer.alert("请选择商品", {
