...
//							select = step.saveArray(select,obj);
		...
		if (bCheck == true && arrayInfor != null && arrayInfor.length > 0) {
			...
			var itemColumHead = $("<th class=\"th-price\" width=\"13%\">规格价(元)</th><th class=\"th-code\" width=\"10%\">库存</th>");
			itemColumHead.appendTo(trHead);
				...
				$.each(zuheDate,function(index, item) {
					...
					if(specPriceObj != null){
						...
						if(obj != null){
							...
							var maxPrice = $("#depositPercent").attr("maxPrice");
							if(maxPrice != null && maxPrice != "" && typeof(maxPrice) != "undefined"){
								if(price*1 < maxPrice){
									$("#depositPercent").attr("maxPrice",price);
								}
							}
					...
					var td1 = $("<td  class='inp'>"+price+"<input type='hidden' class='invPrice' value='"+price+"'/></td>");
					td1.appendTo(tr);
					if(minPrice > price || minPrice == 0){
						minPrice = price;
					}
					/*var td2 = $("<td  class='inp'>"+price+"</td>");
					td2.appendTo(tr);*/
					
					var td3 = $("<td  class='inp'>"+num+"</td>");
					td3.appendTo(tr);
					
					specArray[specArray.length] = {
						"invNum" : num,
						"specId" : specId,
						"invenId" : invenIds
					};
