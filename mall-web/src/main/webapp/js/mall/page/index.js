...
function del(ids) {
    ...
    layer.confirm('确认要删除此数据吗?删除之后，引用该页面的地方将会因为找不到该链接跳转到404页面', {icon: 3, title: '提示', shade:[0.1,'#fff'], offset: "40%"}, function (index) {
        layer.close(index);
        var params = {
            ids: ids
        };
...
function setMian(id, shopid) {
    layer.confirm('确认要设为主页吗?', {icon: 3, title: '提示', shade:[0.1,'#fff'], offset: "40%"}, function (index) {
        layer.close(index);
        var params = {
            id: id,
            shopid: shopid
