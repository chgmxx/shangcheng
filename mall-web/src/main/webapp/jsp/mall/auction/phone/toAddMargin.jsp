...
<head>
    ...
    <title>${mapmessage.pro_name }-交纳保证金</title>
...
<link id="link" rel="stylesheet" type="text/css" href="/css/mall/auction/margin.css?<%=System.currentTimeMillis()%>"/>
...
<div class="Warp">
    <h1>用户竞拍服务协议</h1>
    <article>
        1.0.0用户应按照竞拍规则及商家设置的保证金金额进行缴纳，且在竞拍成功后应在规定时间内完成付款（用户付款前应下载并查看成交确认书，并于提取成交拍品前与商家签署成交确认书）。用户逾期不付清成交款、佣金或其他费用的，自逾期之日起，商家每日按逾期支付金额的千分之五计收取滞纳金；同时，自用户未按约定期限支付任一期款项之日的次日起，商家有权即时单方面解除双方签订的《拍卖成交确认书》，不予退还用户缴纳的保证金，追究用户的违约责任并要求用户赔偿商家及委托人因此所遭受的实际经济损失，包括但不限于：如竞拍标的再行组织竞拍活动，原获拍用户应当支付第一次竞拍活动中商家及委托人应当向商家支付的拍卖佣金；再行组织竞拍的成交价款低于原竞拍价款的，原获拍用户应当补足差额；并原获拍用户应当支付再次竞拍所产生的相关费用。
        用户逾期不付清款项而造成违约的，系统将直接扣除用户缴纳的保证金作为违约金用于赔付商家；商家应向用户出示相应凭据，但无需为扣除的保证金给予用户开具发票或收据等凭证。至于其他的费用，如滞纳金、经济损失、佣金、差额费用等金额的追缴及法律责任的追究，由商家线下与用户自行联系完成。所涉款项不通过平台支付，平台因此不对此承担任何义务和法律责任。
        ...
        1.1.1 竞拍保证金(简称“保证金”或“佣金”)须知：
        1.1.2 保证金的缴纳：保证金是竞拍人参加竞拍的凭证，如竞拍人有意参加相关商品竞拍活动，则须缴纳商家设置的参与竞拍活动的保证金。每个商品的竞拍只需缴纳一次保证金。
        1.1.3 保证金的处理：
        1.1.4保证金的返还以下三种情况下，将原路退还用户保证金：
        1.1.4.1用户未成功竞拍商品；
        1.1.4.2用户竞拍成功后并在竞拍规则规定的时间内付款；
        1.1.4.3用户竞拍成功后，商家主动关闭交易或未履约发货或因商家其他原因导致交易未完成的。
    </article>
    <div class="agree"><input type="checkbox" name="" id="agrea" value="" checked="checked"/>我已阅读并同意协议</div>
    <div class="info">
        <h2>订单信息</h2>
        <div class="info-item flex">
            <div class="flex-1">拍品名称：</div>
            <div class="flex-3">${mapmessage.pro_name }</div>
        </div>
        <div class="info-item flex" onclick="showLay()">
            <div class="flex-1">支付方式：</div>
            <c:if test="${isWxPay == 1}">
                <div class="flex-3" id="onlinePayment">微信支付 ></div>
            </c:if>
            <c:if test="${isWxPay == 0 && isAliPay == 1}">
                <div class="flex-3" id="onlinePayment">支付宝支付 ></div>
            </c:if>
        </div>
    </div>
    <form method="post" id="marginForm">
        <input type="hidden" class="specIds" name="proSpecificaIds" value="${guige.xids }"/>
        ...
        <input type="hidden" class="proId" name="proId" value="${mapmessage.id }"/>
        <input type="hidden" class="aucId" name="aucId" value="${auction.id }"/>
        <input type="hidden" class="marginMoney" name="marginMoney" value="${auction.aucMargin }"/>
        ...
        <div class="text-right">
            <input type="button" class="recog-btn" name="" id="" value="交保证金"/>
        </div>
    </form>
...
<script type="text/javascript" src="/js/mall/auction/phone/margin.js"></script>
