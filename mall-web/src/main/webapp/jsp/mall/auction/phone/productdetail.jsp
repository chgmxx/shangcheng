...
<head>
    ...
    <style type="text/css">
        ...
        .tip .tip_left {
            ...
            width: 50px;
            text-align: center;
            background: url("/images/mall/img/black-up.png") no-repeat center center;
            background-size: 28%;
        }
...
<div class="Warp">
    <header>
        <div class="tip">
			<span class="tip_left" onclick="javascript:location.href='/mAuction/${id }/${shopid }/${auctionId }/79B4DE7C/auctiondetail.do';">
			</span>
            <span class="tip_right" onclick="pageclick('${pageid}')">首页</span>
        </div>
        <ul class="flex" class="">
            <li class="flex-1 <c:if test='${type == 1 }'>cur</c:if>" id="section1">
                <a href="/mAuction/${id }/${shopid }/${auctionId }/79B4DE7C/shopdetails.do?type=1">商品详情</a>
            </li>
            <li class="flex-1 <c:if test='${type == 2 }'>cur</c:if>" id="section2">
                <a href="/mAuction/${id }/${shopid }/${auctionId }/79B4DE7C/shopdetails.do?type=2">
                    <c:if test="${aucType == 1}">抢拍记录</c:if>
                    <c:if test="${aucType == 2}">出价记录</c:if>
                </a>
            </li>
            <li class="flex-1 <c:if test='${type == 3 }'>cur</c:if>" id="section3">
                <a href="/mAuction/${id }/${shopid }/${auctionId }/79B4DE7C/shopdetails.do?type=3">卖家承诺</a>
            </li>
            <li class="flex-1 <c:if test='${type == 4 }'>cur</c:if>" id="section4">
                <a href="/mAuction/${id }/${shopid }/${auctionId }/79B4DE7C/shopdetails.do?type=4">保证金须知</a>
            </li>
        </ul>
    </header>
    <c:if test='${type == 1 }'>
