    				...
    				<div class="order-filter">
    					<div class="fl">
    						<select id="selectType">
    							<option <c:if test="${orderType == 0}">selected</c:if> value="0">所有订单</option>
    							<option <c:if test="${orderType == -1}">selected</c:if>  value="1">退款订单</option>
    					...
    					<div class="fl">
    						<select id="filter" onchange="searchOrderFilter(this);">
    							<option <c:if test="${orderFilter == 0}">selected</c:if> value="0">请选择</option>
    							<option <c:if test="${orderFilter == 1}">selected</c:if> value="1">订单号</option>
    							<%-- <option <c:if test="${orderFilter == 2}">selected</c:if> value="2">外部订单号</option> --%>
    							<option <c:if test="${orderFilter == 3}">selected</c:if> value="3">收货人姓名</option>
    							<option <c:if test="${orderFilter == 4}">selected</c:if> value="4">收货人手机</option>
    							<option <c:if test="${orderFilter == 5}">selected</c:if> value="5">微信昵称</option>
    						</select>
						            ...
						            <div class="td-cont">
						                <p class="js-order-state">
						                	...
						                	<c:if test="${order.orderStatus == 2  && order.deliveryMethod == 1}">待发货
						                		<a id="butShip" onclick="orderRemark(4,this,${order.groupBuyId==null?0:order.groupBuyId });"  style="cursor: pointer;">发货</a>
						                	</c:if>
				    ...
				    <c:if test="${order.orderPayWay == 5 }"><!-- 扫码支付 -->
				    	<tr class="content-row">
					        ...
					        <td class="customer-cell" rowspan="">
					           <p>${order.userName}</p>
					           <p class="user-name">${order.recUserName}</p>
					            ${order.recTelephone}
					        </td>
...
//		parent.openIframeNoScoll("同意退款退货","500px","380px","/mallOrder/returnPopUp.do?rId="+rId+"&type=2"+"&oNo="+oNo+"&orderPayNo="+orderPayNo);
	...
	$(".realReturn").click(function(){
		...
		var oNo = $(this).attr("oNo");
		var orderPayNo = $(this).attr("orderPayNo");
		parent.openIframeNoScoll("确认收货并退款","500px","400px","/mallOrder/returnPopUp.do?rId="+rId+"&type=3"+"&oNo="+oNo+"&orderPayNo="+orderPayNo);
	});
	/**
	 * 拒绝确认收货
	 */
