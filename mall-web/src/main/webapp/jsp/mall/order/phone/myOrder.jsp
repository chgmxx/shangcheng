...
body,html{height:100%;width: 100%;font-size: 30px; position: relative;}
.Warp{width: 870px;min-height: 100%;background-color: #f0f2f5;margin: 0 auto;color: #484848;margin-bottom: 30px;}
.nav{display: block;width: 100%;background-color: #FFFFFF;border-bottom: 1px solid #e1e1e1;line-height: 60px;
 text-align: center; /* margin-bottom: 20px; height: 60px;*/}
.nav ul{width: 100%;}
.nav ul li{width: 20%;height: 58px;line-height: 58px;float: left;box-sizing: border-box;text-align: center;list-style: none;}
.nav ul li a{display: inline-block;width: 100%;}
.nav .active{color: #f20000;border-bottom: 2px solid #F20000;}
.main ul{width: 100%;}
.main ul li{background-color: #FFFFFF;list-style: none;/* margin-top: 40px; */}
.main .mall-item{border-bottom: 1px solid #eeeeee;padding: 15px 40px 0px 40px;}
.main .mall-img{display: inline-block;width: 165px;}
.main .mall-img img{width: 123px;max-height: 162px;display: block;margin: 0 auto;}
.main .mall-info{display: inline-block;width: 350px;vertical-align: top;font-size: 28px;}
.main .mall-info .red-txt{color: #F20000;font-size: 30px;}
.main .mall-info span{margin-right: 40px;}
.main .mall-info p.title{height: 40px;overflow: hidden;text-overflow: ellipsis; display: -webkit-box;-webkit-line-clamp: 2;
    -webkit-box-orient: vertical;width: 470px;}
.mall-status{height: 85px;line-height: 85px;position: relative;border-bottom: 1px solid #eeeeee;background-color:#fff;}
.mall-status label{	padding-left: 20px;}
.mall-status .goPay{float:right;padding:0px 10px;height: 48px;border-radius: 10px;margin-top:15px;/* position: absolute;top: 50%;right: 30px;margin-top: -24px; */
  	text-align: center;border: 2px solid #F20000;font-size: 28px;display: inline-block;line-height: 48px;color: #F20000}
.mall-item .goReturn,.btn{height: 48px;border-radius: 10px;text-align: center;border: 2px solid #F20000;font-size: 28px;line-height: 48px;
  	color: #F20000;vertical-align:top;padding:0px 10px;}
.mall-item .div_a{display:inline-block;width:250px;vertical-align:top;	}
.mall-item .div_a p{text-align:right;margin-top:10px;}
.mall_order_no{height:90px;line-height:90px;/* background:#f0f2f5; */border-bottom: 1px solid #eeeeee;}
.mall_order_no span{margin-left: 20px;}
.df_a{float:right;margin-top:15px;margin-right:10px;cursor: pointer;}
/* 添加弹出层 */
.gt_flex{display: -webkit-box;display: -webkit-flex;display: flex;}
.gt_dialog .gt_bg,.gt_dialog .gt_box{position:fixed;}
.gt_dialog .gt_bg{top: 0;left: 0;right: 0;bottom: 0;background-color: rgba(0, 0, 0, 0.6);z-index: 9990}
.gt_dialog .gt_imgbg{background-color: rgba(255, 255, 255, 0.8);}
.gt_dialog .gt_box{background: #fff; width: 86%;border-radius: 12px;top:100px;left: 7%;overflow: hidden;z-index: 9991}
.gt_dialog .gt_box .st_label{font-size:34px;color: #1eb1f4;text-align: center;margin: 50px 0 45px 0;}
.gt_dialog .st_html{font-size: 32px;text-align: center;} 
.gt_dialog .gt_box .st_btn{margin-top:3rem;border-top:solid 1px #d2d2d2}
.gt_dialog .gt_box .st_btn a{position: relative; display: block; -webkit-box-flex: 1;-webkit-flex: 1;flex: 1;text-align: center;height: 100px;line-height: 100px;text-align: center;font-size: 32px;color: #333}
.gt_dialog .gt_box .st_btn a:last-child:before{position: absolute; display: block;content: "";height: 4.4rem;width: 1px;background-color:#d2d2d2;top:0;left: -1px;}
.gt_dialog .st_tips_txt{display: inline-block;width: 80%;line-height: 1.6;color: #666}
			...
			<li>
				...
				<div class="mall-item" <c:if test="${order.orderPayWay != 5}">onclick="toOrderDetail(${order.id});"</c:if>>
 				...
 				<c:if test="${!empty orderDetail.isReturnButton }">
 					<p><a href="/phoneOrder/79B4DE7C/toReturn.do?dId=${orderDetail.id }&id=${orderDetail.orderReturn.id }&type=0&uId=${userid}" class="goReturn" id="goReturn">申请退款</a></p>
 				</c:if>
 				<c:if test="${!empty orderDetail.isUpdateButton }">
 					<p><a href="/phoneOrder/79B4DE7C/toReturn.do?dId=${orderDetail.id }&id=${orderDetail.orderReturn.id }&type=0&uId=${userid}" class="goReturn" id="goReturn">修改退款申请</a></p>
 				</c:if>
 				<c:if test="${!empty orderDetail.isCancelButton }">
 					<p><a href="javascript:void(0);" class="goReturn" id="closeReturn" rId="${orderDetail.orderReturn.id }" dId="${orderDetail.id }" oId="${order.id }">撤销退款</a></p>
					<c:set var="isComment" value="1"></c:set>
 				</c:if>
 				<c:if test="${!empty orderDetail.isWuliuButton }">
 					<p><a href="/phoneOrder/79B4DE7C/toReturn.do?dId=${orderDetail.id }&id=${orderDetail.orderReturn.id }&type=1&uId=${userid}" class="goReturn">填写退货物流信息</a></p>
					<c:set var="isComment" value="1"></c:set>
 				</c:if>
...
<input type="text" class="pageCount" value="${page.pageCount }"/>
<input type="text" class="isNextPage" value="${page.isNextPage }"/>
<input type="text" class="curPage" value="${page.curPage }"/>
