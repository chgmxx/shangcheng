	  	...
	  	<div>
			<div>
		  		...
		  		<div class="info info2">
		  			收货地址：${address.pName}${address.cName}${address.aName}${address.mem_address }
			...
			<div class="mall-info">
				...
				<p class="red-txt" style="color: #3f3f3f;">x${orderDetail.detProNum }</p>
 				<c:if test="${!empty orderDetail.isReturnButton }">
 					<p class="red-txt"><a class="order_apply goReturn" href="/phoneOrder/79B4DE7C/toReturn.do?dId=${orderDetail.id }&id=${orderDetail.orderReturn.id }&type=0&uId=${userid }">申请退款</a></p>
 				</c:if>
 				<c:if test="${!empty orderDetail.isUpdateButton }">
 					<p class="red-txt"><a href="/phoneOrder/79B4DE7C/toReturn.do?dId=${orderDetail.id }&id=${orderDetail.orderReturn.id }&type=0&uId=${userid}" class="goReturn" id="goReturn">修改退款申请</a></p>
 				</c:if>
 				<c:if test="${!empty orderDetail.isCancelButton }">
 					<p class="red-txt"><a href="javascript:void(0);" class="goReturn order_apply closeReturn" id="closeReturn" rId="${orderDetail.orderReturn.id }" dId="${orderDetail.id }" oId="${order.id }">撤销退款</a></p>
 				</c:if>
 				<c:if test="${!empty orderDetail.isWuliuButton }">
 					<p class="red-txt"><a href="/phoneOrder/79B4DE7C/toReturn.do?dId=${orderDetail.id }&id=${orderDetail.orderReturn.id }&type=1&uId=${userid }" class="goReturn order_apply" style="width:250px;">填写退货物流信息</a>
 				</c:if>
				<c:if test="${order.orderStatus  == 4 && mallPaySet.isComment == 1 && isComment == 0 && orderDetail.appraiseStatus == 0}">
					<p class="red-txt"><p><a href="/mMember/79B4DE7C/orderAppraise.do?orDetailId=${orderDetail.id }&uId=${userid}" class="goReturn">去评价</a></p>
				</c:if>
			</div>
		</div>
	</c:forEach>
...
var interval = 1000; 
	...
	if(null != groupBuyTime && groupBuyTime != ""){
		window.setInterval(function(){ShowCountDown(groupBuyTime,'time');}, interval); 
	}
