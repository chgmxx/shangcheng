    ...
    <div class="modal">
        ...
        <div class="modal-body">
			    <textarea class="js-remark" rows="4" placeholder="最多可输入256个字符" maxlength="256"
                          style="width: 396px;" id="orderSellerRemark">${result.orderInfo.order_seller_remark}</textarea>
...
<c:if test="${type == 2 }">
    <div class="modal" style="width: 262px;margin-left: -125px;">
        ...
        <div class="modal-body">
            <select id="reason">
                <c:if test="${result.orderInfo.seller_cancel_reason == null || result.orderInfo.seller_cancel_reason != '' }">
                    <option value="0">请选择一个取消订单的理由</option>
                </c:if>
                <c:forEach var="orderReason" items="${cancelReason }">
                    <option ${orderReason.item_key == result.orderInfo.seller_cancel_reason?'selected':''}
    ...
    <div class="modal" style="width: 652px;height: 300px;margin-left: -320px; position: absolute;">
        ...
        <div class="modal-body">
                            ...
                            <td class="tb-discount">
                                ...
                                <%-- <c:if test="${s.count==1}">
                                    <td class="tb-discount" rowspan="${fn:length(result.orderDetail)}">
                                        <input type="text" id="orderMoney" name="orderMoney" value="${result.orderInfo.order_money }">
                                    </td>
                                </c:if> --%>
                        </tr>
                    </c:forEach>
            ...
            <div class="final js-footer text-left">
                <div>
                    <p>收货地址： ${result.orderInfo.pName}${result.orderInfo.cName}${result.orderInfo.aName}${result.orderInfo.mem_address },
                            ${result.orderInfo.mem_name }(收),${result.orderInfo.mem_phone }</p>
                    <p>买家实付：${result.orderInfo.order_money }元
                        <c:if test="${result.orderInfo.order_freight_money != null && result.orderInfo.order_freight_money != ''&& result.orderInfo.order_freight_money > 0 }">
                            <span style="color: red;">(含运费${result.orderInfo.order_freight_money })</span>
                        </c:if>
                    </p>
                </div>
            </div>
            ...
            <div class="modal-body">
                            ...
                            <tr>
                                ...
                                <c:if test="${s.count==1}">
                                    <td rowspan="${fn:length(result.orderDetail)}" style="border-right: 1px solid #E4E4E4;">
                                            ${result.orderInfo.order_no }
                                    </td>
                                    <td rowspan="${fn:length(result.orderDetail)}">${result.orderInfo.order_money }</td>
                                </c:if>
                            </tr>
                        </c:forEach>
                        </tbody>
                ...
                <div style="font-size: 14px;margin-top: 15px;">
                    <div class="order_ship" style="width:545px;height:auto;line-height: 25px;">
                        ...
                        <div class="fl" style="width: 80%;">${result.orderInfo.pName}${result.orderInfo.cName}${result.orderInfo.aName}${result.orderInfo.mem_address },
                                ${result.orderInfo.mem_name }(收),${result.orderInfo.mem_phone }</div>
                    </div>
                            ...
                            <div class="fl">
                                <select id="logisticsCompany">
                                    <c:forEach var="company" items="${logisticsCompany }">
                                        <option ${company.item_key == result.orderInfo.express_id?'selected':''}
