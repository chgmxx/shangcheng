...
<div class="Warp">
	   ...
	   <c:if test="${empty groupName }">预售商品</c:if>
	    <%-- <c:if test="${groupId eq null}">  预售商品</c:if>
	   ...
	   <c:set var="urls" value="/phonePresale/${shopid }/79B4DE7C/presaleall.do"></c:set>
	   <input type="hidden" class="urls" value="${urls }"/>
		...
		<section class="list-tit w"  style="margin-bottom: 0px;">
		    <ul  class="flex">
		        ...
		        <li class="flex-1" onclick="queryurl(3,'${desc}','${groupId}',0)"><i  class="<c:if test="${type eq 3 }">current </c:if>">价格<c:if test="${type eq 3 && desc eq 0 }"><i class="arrow-down"></i></c:if><c:if test="${type eq 3 && desc eq 1}"><i class="arrow-up"></i></c:if></i></li>
					   ...
					   <div class="product-info">
					        <div class="info-title">
					         ...
					         <div class="info-price flex">
					        	<div class="now-price">
					        		<!-- 当前价: -->￥<span class="now-price">${list.price}</span>                                                       
					        	</div>
					        </div>
					         <div class="info-price flex">
					        	 <div class="info-price flex">
					        ...
					        <div class="time-left flex <c:if test="${list.status ==0}">endFlex</c:if>" >
					        	...
					        	<div class="count-down time-item" status="${list.status }" style="padding-left: 0.1rem;">
...
<script type="text/javascript" src="/js/mall/product/shoppingall.js"></script>
	...
	<script type="text/javascript">
		...
		function queryurl(type,desc,style,status){
			...
			var url = "/phonePresale/${shopId}/79B4DE7C/presaleall.do?type="+type+"&desc="+desc;
			if(style!=''){
				url += "&groupId="+style;
			}
			if(status == 0){
