...
<link rel="stylesheet" type="text/css" href="css/trade/mall/manageList.css"/>
<link rel="stylesheet" type="text/css" href="css/trade/mall/group.css" />
...
<script type="text/javascript">
	...
	if(top==self){
		 window.location.href="/mallWholesalers/start.do";
	}
	$(function(){
		/* $(".copy").zclip({ 
	...
	<div id="con-box">
		...
		<div class="msg-list">
			<ul id="list group_ul">
				...
				<c:if test="${!empty page.subList}">
					...
					<li class="txt-tle" style="min-height:50px;height:auto;">
						...
						<span class="f2 fl" style="line-height:20px;padding-top:10px;"><fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${pifa.createTime }" /></span>
						<span class="f2 fl span_a"  style="width: 130px;position:relative;display:inline-block;">
							...
							<c:if test="${pifa.status == 1}">
								...
								<a href="javascript:;" class="bj-a copy copy_public" title="复制链接"  data-clipboard-text="${path }/mallPage/${pifa.productId}/${pifa.shopId }/79B4DE7C/phoneProduct.do"  aria-label="复制成功！"></a>
							</c:if>
						</span>
			...
			<c:if test="${! empty page.subList }">
				...
				<jsp:include page="../../../user/pageTwo.jsp"></jsp:include>
			</c:if>
		</div>
		</c:if>
		<c:if test="${empty shoplist }">
...
<script type="text/javascript" src="/js/trade/mall/pifa/pifa.js"></script>
</body>
</html>