...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/css/trade/mall/wholesalers/pcReset.css?<%=System.currentTimeMillis()%>"/>
		<link rel="stylesheet" type="text/css" href="/css/trade/mall/wholesalers/pcWholesale.css?<%=System.currentTimeMillis()%>"/>
        <script src="/js/jquery.min.js?<%=System.currentTimeMillis()%>"></script>
				...
				<c:forEach var="wholesaler" items="${page.subList }">
					<tr>
						...
						<td><c:if test="${empty wholesaler.money }">0</c:if>
							<c:if test="${!empty wholesaler.money }">${wholesaler.money}</c:if>(元)</td>
						<td>
