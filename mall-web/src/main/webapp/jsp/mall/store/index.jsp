...
<html>
  ...
  <script type="text/javascript" src="/js/mall/mall_public.js"></script>
	<script type="text/javascript" src="/js/mall/store/index.js"></script>
			...
			<div class="txt-tle2" id="list">
				<c:forEach items="${page.subList}" var="sto">
					<li>
						...
						<div class="listb" style="width:22%;margin-right: 10px; <c:if test="${fn:length(sto.sto_address)>14}">line-height: 30px;</c:if>
						...
						<div class="listd" style="width: 210px;">
							<div class="bianji" style="width: 220px;position:relative;display:inline-block;">
								<a href="store/to_edit.do?id=${sto.id}"
									class="bj-a textEdit"></a> <a href="javascript:void(0)"
									class="bj-a textDelete" onclick="del(${sto.id})"></a>
									<%-- <a href="javascript:void(0);" class="bj-a bj-record" 
									onclick="viewQR(${sto.id })" title="二维码预览">
									</a> --%>
									
									<c:if test="${!(wxPublicUsers.mchId==null||wxPublicUsers.mchId==''||wxPublicUsers.apiKey==null||wxPublicUsers.apiKey=='')}">
										<a  target="_blank" class="bj-a download"  href="wxPay/3/${sto.id}/createPayQR.do?busId=${sto.sto_user_id}" title="下载支付二维码">
									</a>
									</c:if>
								...
								<a href="javascript:;" class="bj-a copy copy_public" title="复制链接"  data-clipboard-text="${path }/mallPage/${sto.pageId}/79B4DE7C/viewHomepage.do"  aria-label="复制成功！"></a>
								</c:if>
							</div>
							<c:if test="${sto.pageId != null && sto.pageId != '' }">
