...
<div class="wWrapper">
    <nav class="sc-nav">
        <ul class="nav-ul clearfix">
            <li class="nav-li">
                <a href="/mallPage/79B4DE7C/shoppingcare.do?member_id=${memberId }&uId=${userid}" class="nav-a <c:if test='${type == 0 }'>nav-active</c:if>" item="1"
                   onclick="switchNav(this)">购物车</a>
            </li>
            <li class="nav-li">
                <a href="/mallPage/79B4DE7C/shoppingcare.do?member_id=${memberId }&type=1&uId=${userid}" class="nav-a <c:if test='${type == 1 }'>nav-active</c:if>" item="2"
                   id="ws-nav" onclick="switchNav(this)">批发购物车</a>
            </li>
        </ul>
    </nav>
                ...
                <div class="box-list">
                    <c:if test="${!empty shop.proList }">
                        <c:forEach var="pro" items="${shop.proList }">
                            <div class="box-main df <c:if test='${pro.pro_type == 2 || pro.pro_type == 0}'>box-mixed</c:if> <c:if test='${pro.pro_type == 1}'>box-hand</c:if>"
                                 <c:if test="${pro.pro_type == 1}">over="0"</c:if>
                                 types="${pro.pro_type }">
                                    ...
                                    <div class="item-info df">
                                        ...
                                        <div class="item-text clearfix">
                                            <div class="item-text-left"
                                                 <c:if test="${pro.pro_type != 0 }">style="width:auto;float:none;"</c:if> >
                                                <!-- 商品名称 -->
                                        ...
                                        <div class="item-spec ">
                                            <c:if test="${pro.proSpecStr != null }">
                                                <c:forEach var="spec" items="${pro.proSpecStr }" varStatus="i">
                                                    ...
                                                    <span class="pro_spec_div" specIds="${spec.key }" values="${specVal.value }" proType="${pro.pro_type }"
                                                          names="${specVal.specName }">
			                     	${specVal.value } x <span class="item-num mgr">${specVal.num }</span>&yen;<span class="item-price">${specVal.price }</span><input type="hidden"
                                                                                                                                                                      class="item-stock"
                                                                                                                                                                      value="${specVal.invNum }">
			                     </span>
                                    ...
                                    <c:if test="${pro.pro_type == 2 && pro.proSpecStr != null}">
                                        <div class="item-detail">
                                            <c:if test="${pro.proSpecStr != null }">
                                                <c:forEach var="spec" items="${pro.proSpecStr }" varStatus="i">
                                                    <c:set var="specVal" value="${spec.value }"></c:set>
                                                    <div class="df _spec pro_spec_div" specIds="${spec.key }" values="${specVal.value }" proType="${pro.pro_type }"
                                                         names="${specVal.specName }">
		                                    ...
		                                    <i class="iProp minus-icon noDelay" onclick="minusOper(this)"></i>
		                                    <input type="text" class="item-input num pro_inp" value="${specVal.num}" stock="${specVal.invNum }" proid="${pro.product_id }"
                                                   id="${pro.id }"
                                                   max="${!empty pro.maxBuy?pro.maxBuy:0 }" onclick="showLayer(this)">
		                                    <i class="iProp add-icon noDelay" onclick="addOper(this)"></i>
		                                </span>
                                    ...
                                    <div class="item-detail">
                                        <c:if test="${pro.proSpecStr != null }">
                                            <c:forEach var="spec" items="${pro.proSpecStr }" varStatus="i">
                                                ...
                                                <span>${specVal.value } x<span class="item-num mgr">${specVal.num }</span>&yen;<span
                                                        class="item-price">${specVal.price }</span><input type="hidden" class="item-stock" value="5"></span>
                                                <c:if test="${i.index+1 <  pro.proSpecStr.size()}">、</c:if>
                                            </c:forEach>
                                        </c:if>
                                    </div>
