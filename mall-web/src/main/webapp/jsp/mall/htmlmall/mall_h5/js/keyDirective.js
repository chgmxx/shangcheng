				...
				new Sortable(element.find(".page")[0],{
					...
					onEnd: function (/**Event*/evt) { // 拖拽
						$scope.$apply(function(){
							...
							dataTransverse.changeAt(start_index,$(evt.item).index());
						});
						$timeout(function(){
				...
				switch (key)
				{
					...
					case 67:
						if(!$scope.showBg && event.ctrlKey && !$("textarea,input").is(":focus")){
							$scope.$apply(function(){
								$scope.copy = angular.toJson($scope.MAIN_SELECT);
						...
						if(event.ctrlKey){
							$scope.$apply(function() {
								if($scope.copy && !$("textarea,input").is(":focus")){
									...
									$scope.NAV_SELECT[len - 1] = eval('(' + $scope.copy + ')');
									var data = $scope.NAV_SELECT[len - 1].attrs;
									data.zIndex = num;
