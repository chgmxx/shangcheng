...
angular.module('app', ['data','dragModule', 'dragPlan', 'colorTo', 'farbtastic', 'json', 'scalescreen', 'keydown', 'cutpic' , 'buttongo', 'music','form','map','transverse',"ts","bgswiper",'htmljs'])
    .controller('adminController', ['$scope', '$timeout', 'colorToService', 'jsonService','Data', function ($scope, $timeout, colorToService, jsonService, Data) {
        ...
        //横向滑动数据
        $scope.dataTransverse = Data.dataTransverse;
        //数据库图片
        ...
        $scope.setMainSelectIndex = function (index) {
            ...
            $("#dashboard .box-controller").removeClass("u-comChoose").eq($scope.MAIN_SELECT_index).addClass("u-comChoose");
        };
        ...
        $scope.removeLayer = function () {
            ...
            jsonService._delete($scope.NAV_SELECT, $scope.MAIN_SELECT_index);
            $timeout(function () {
                var obj = $("#dashboard .box-controller");
                var len = obj.length;
                var domArray = [];
                var zIndexArray = [];
...
/**********************************************************************************************************************/
        ...
        $scope.addmap = function(){
            var len = $scope.NAV_SELECT_length = $scope.NAV_SELECT.length + 1;
            var num = len * 100 + 10000;
            $scope.NAV_SELECT[len - 1] = {
                "style": "",
                "attrs": {
                    "width": 39,
                    "height": 56,
        ...
        $scope.addbutton = function(param){
            ...
            $scope.NAV_SELECT[len - 1] = {
                ...
                "phone":"",
                "QQ":"",
                "swiper":["images/18d8bc3eb13533fa33395e46aed3fd1f40345bcf.gif","images/CoQ8n1ZFW-mAROSoAAKOWcJWeSQ733.png","images/code_ewf.png","images/phone1_bg.png"],
                "bgcolor":color,
                "bgopacity":90,
                "dataPic":{
                    "height": 75,
        ...
        $scope.addNewPic = function () {
            ...
            if (provenance == 'pic') {
                ...
                if($scope.horizontal_page)$scope.dataTransverse[$scope.NAV_SELECT_index].bg[$scope.dataTransverse_index].background = "url(" + src + ")";
                else $scope.dataModuleBg[this.NAV_SELECT_index] = {background: "url(" + src + ")"};
...
/**********************************************************************************************************************/
        ...
        $scope.countHeight = function (param) {
            ...
            $timeout(function(){
                $scope.MAIN_SELECT.attrs.height = $("#dashboard .box-controller").eq($scope.MAIN_SELECT_index).find(".textbox").height() / $scope.scale;
            })
        };

        //取色板背景
        $scope.showHideBgColorBoard = function ($event) {
            if($scope.horizontal_page){
                if ($scope.bghBoard) {
                    $scope.bghBoard = false;
                    return false;
                }
                $('#pickerh').farbtastic('#bgh');
                $scope.bghBoard = true;
                $timeout(function () {
                    $("#bgh").keyup();
                });
        ...
        $scope.editBgColor = function (arguments) {
            if($scope.horizontal_page)$scope.dataTransverse[$scope.NAV_SELECT_index].bg[$scope.dataTransverse_index].background = arguments;
            else $scope.dataModuleBg[$scope.NAV_SELECT_index].background = arguments;
        };
        ...
        $scope.addpage = function () {
            ...
            $scope.dataTransverse.splice($scope.NAV_SELECT_index + 1,0,{'data':[[],[],[]],'bg':[{ 'background':'#fff',},{ 'background':'#fff',},{ 'background':'#fff',}],'attr':{'width': 380,'height': 500,'top': 62,'left': 0,},'dataTransverse_on':false})
            $timeout(function(){
        ...
        $scope.copy_addpage = function(){
        	jsonService._add($scope.dataJson, $scope.NAV_SELECT_index + 1);
            jsonService._add($scope.dataModuleBg, $scope.NAV_SELECT_index + 1,true);
            jsonService._add($scope.dataTransverse, $scope.NAV_SELECT_index + 1,true);
            ...
            $scope.dataTransverse[$scope.NAV_SELECT_index + 1] = angular.fromJson(angular.toJson($scope.dataTransverse[$scope.NAV_SELECT_index]));
            $timeout(function(){
                $scope.setNavSelectIndex($scope.NAV_SELECT_index + 1);
            })
        };
        ...
        $scope.removepage = function ($index) {
            ...
            jsonService._delete($scope.dataTransverse, $index);
            $timeout(function(){
                if ($scope.dataJson.length <= $index)$index = $scope.dataJson.length - 1;
                $scope.setNavSelectIndex($index);
            })
        };
        //改变点击序号
        ...
        $scope.setJson = function(){
            $scope.setNavSelectIndex(arguments[0]);
            $scope.horizontal_page = true;
            $scope.dataTransverse_index = arguments[1];
            $scope.NAV_SELECT = $scope.dataTransverse[$scope.NAV_SELECT_index].data[$scope.dataTransverse_index];
        };

        $scope.set_dataTransverse_on = function(){
          $scope.dataTransverse[$scope.NAV_SELECT_index].dataTransverse_on = !$scope.dataTransverse[$scope.NAV_SELECT_index].dataTransverse_on;
        };

        $scope.choosebg = function(){
            if($scope.horizontal_page)return;
            return $scope.dataModuleBg[$scope.NAV_SELECT_index].background;
        };


        $scope.savedebug = function(){
            console.log($scope);
            $scope.NAV_SELECT = "";
        ...
        $scope.addbgswiperpic = function($event){
        	var json = eval($($event.target).val());
        	if(!$scope.dataModuleBg[$scope.NAV_SELECT_index].hasOwnProperty("bgswiperpics"))
    		$scope.dataModuleBg[$scope.NAV_SELECT_index].bgswiperpics=[];
        	json.forEach(function(e){
        		$scope.dataModuleBg[$scope.NAV_SELECT_index].bgswiperpics.push(e.url);
        	})
        };
        $scope.removebgswiperpic = function(){
            $scope.dataModuleBg[$scope.NAV_SELECT_index].bgswiperpics.splice(arguments[0],1);
        };
        $scope.setsortingbgswiperpic = function(){
            if(arguments[1]<0){
                arguments[1] = $scope.dataModuleBg[$scope.NAV_SELECT_index].bgswiperpics.length-1;
            }
            if(arguments[1]>=$scope.dataModuleBg[$scope.NAV_SELECT_index].bgswiperpics.length){
                arguments[1] = 0;
            }
            $scope.dataModuleBg[$scope.NAV_SELECT_index].bgswiperpics.changeAt(arguments[0],arguments[1]);
        }
